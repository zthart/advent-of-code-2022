...
class ElfSnacks():
    ...
    def get_sum_of_top_n(self, n):
        return functools.reduce(lambda acc, n: acc+n, heapq.nlargest(n, self._heap))

def main():
    snacks = ElfSnacks()
    running_total = 0
    with open(os.path.join(os.path.dirname(__file__), 'input.txt'), 'r') as in_file:
        for line in in_file:
            if (cals := line.rstrip()) != '':
...
if __name__ == '__main__':
    all_snacks = main()
    print(f'top:\t{all_snacks.get_top_cals()}\ntop 3:\t{all_snacks.get_sum_of_top_n(3)}')
